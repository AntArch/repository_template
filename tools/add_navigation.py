...
def write_navbars():
    for nb_name, navbar in iter_navbars():
        ...
        #if is_comment(nb.cells[1]):#original placing text in second cell
        if is_comment(nb.cells[0]):
            print("- amending navbar for {0}".format(nb_file))
            #nb.cells[1].source = navbar #original placing text in second cell
            nb.cells[0].source = navbar
        else:
            print("- inserting navbar for {0}".format(nb_file))
            #nb.cells.insert(1, new_markdown_cell(source=navbar)) #original placing text in second cell
